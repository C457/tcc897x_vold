...
static char MKFATFS_PATH[] = "/system/bin/mkfatfs";
#else
...
int Fat::check(const char *fsPath) {
    ...
    do {
        ...
        default:
            SLOGE("FAT: Filesystem check failed (unknown exit code %d)", status);
            errno = EIO;
            return -1;
        }
    } while (0);
...
int Fat::doMount(const char *fsPath, const char *mountPoint,
    ...
    //+NATIVE_PLATFORM
    #ifdef FUNCTION_STORAGE_TUXERA_PATCH    
    rc = mount(fsPath, mountPoint, "tfat", flags, mountData);

    if (rc && errno == EROFS) {
        SLOGE("%s appears to be a read only filesystem - retrying mount RO", fsPath);
        rc = mount(fsPath, mountPoint, "tfat", flags | MS_RDONLY, mountData);
    }
...
int Fat::format(const char *fsPath, unsigned int numSectors, bool wipe) {
    ...
    //+NATIVE_PLATFORM
    #ifdef FUNCTION_STORAGE_TUXERA_PATCH    
    SLOGI("Formatting FAT file system at \"%s\"...", fsPath);
