...
VolumeManager::VolumeManager() {
    ...
    mUmsDirtyRatio = 5; // For telechip 0 -> 5
...
//+NATIVE_PLATFORM
#ifdef FUNCTION_STORAGE_FOR_AUTOMOTIVE
        char *devpath = (*i)->getDevicePath(); // don't need to free
        const char *mntPnt = (*i)->getFuseMountpoint(); // <- changed from getMountpoint for Kitkat
        char *nodepath = (*i)->getNodePath();
        ...
        getDevType(mntPnt, devType);

        int32_t volumeId = 0;
        //+NATIVE_PLATFORM
        #ifdef FUNCTION_STORAGE_TUXERA_PATCH    
        if (getVolumeInfo(mntPnt, nodepath, volumeLabel, sizeof(volumeLabel), &volumeId) != 0) {
        #else
        if (getVolumeInfo(mntPnt, nodepath, volumeLabel, &volumeId) != 0) {
        #endif
        //-NATIVE_PLATFORM
            SLOGE("cannot retreive filesystem from %s", nodepath);
        }

        asprintf(&buffer, "%s %s %d %s %s %d",
                 (*i)->getLabel(), (*i)->getFuseMountpoint(),
                 (*i)->getState(), devType, volumeLabel, volumeId);
...
int VolumeManager::formatVolume(const char *label, const char *fstype, bool wipe) { // For telechips add fstype
    ...
    return v->formatVol(label, fstype, wipe); // For telechips 
}
...
int VolumeManager::unmountLoopImage(const char *id, const char *idHash,
    ...
    for (i = 1; i <= UNMOUNT_RETRIES; i++) {
        rc = umount2(mountPoint, MNT_DETACH); // For telechips
        if (!rc) {
            break;
        }
...
#define VOLD_RETRY_MS (1000 * 1000)
int VolumeManager::mountVolume(const char *label) {
    ...
    for (i=0; i < 60; i++) { // waiting for 1 minute
        ret = v->mountVol();
        if (ret == 0) {
            break;
        }
        SLOGI("VolumeManager::mountVolume:  retry mount count is %d.", i);
        usleep(VOLD_RETRY_MS);
    }
